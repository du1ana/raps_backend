...
router.route("/add").post((req, res) => {
  const { body } = req;
  const { date, name, sessionToken } = body;
  //Data constraints
  if (!date) {
    return res.send({
      success: false,
        ...
        newPublicHoliday
          ...
          .catch((err) =>
            res.send({
              success: false,
              message:
                "Data Validation Error: Date might already be set as holiday.",
            })
          );
      }
    }
...
//Find if today is a public holiday
router.route("/isTodayHoliday").get((req, res) => {
  ...
  PublicHoliday.findOne({ date: filter }, (err, document) => {
    if (err) {
      return res.send({
        success: false,
        message: "Error:Server error",
...
router.route("/update").post((req, res) => {
  const { body } = req;
  const { id, date, name, sessionToken } = body;
  //Data constraints
  if (!date) {
    return res.send({
      success: false,
